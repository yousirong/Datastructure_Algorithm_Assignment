#2) 다음과 같은 개의 행과 개의 열의 셀(cell)들로 이루어진  격자가 있다.
# 여기서 가장 위의 행은 행 1이고 가장 아래의 행은 행 이며,
...
def mngrid_solution(m, n):  # m*n격자 경로 개수 출력 함수
    if 1<= n and m <=2000:
        grd = [[0 for _ in range(n + 1)] for _ in range(m + 1)]   # m*n격자를 생성함 각 셀의 값은 0
        for i in range(1, m + 1):   # for 구문을 m 과 n을 검사함
            for j in range(1, n + 1):
                if i == 1 and j == 1:   #  i = 1과 j = 1 일 경우 격자[1][1] 의 값은 1
                    grd[i][j] = 1
                else:    # 행과 열이 1이 아닐 경우
                    grd[i][j] = grd[i - 1][j] + grd[i][j - 1]  # 부분문제 최적 해 값(목적함수)의 재귀적 정의
        return grd[m][n] % 9999999   # 경로의 개수를 9999999로 나눈 나머지 출력
