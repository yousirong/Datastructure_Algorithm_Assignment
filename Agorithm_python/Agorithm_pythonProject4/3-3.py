...
def bfs(start, arr_visit):  # 탐색 방법 : 너비 우선 탐색
    people = 0  # 사람들
    queue =deque()   # 너비 우선 탐색에서는 큐 이용
    queue.append(start)
    while queue:
        start = queue.popleft()  # 첫 번째 시작값이 큐에 잇을 경우 pop해줌
        if start not in arr_visit:  # 만약 arr_visit에 방문하지 않았을 경우 방문처리
            ...
            for w in arr[start]:  # arr에 관계 시작부분이 있을 경우 append
                queue.append(w)  # stack에 append
...
for i in range(n):  # 입력 받은 n만큼 for구문 돌음
    visited_arr, people = bfs(i, visited_arr)  # 깊이 우선 탐색 돌음
